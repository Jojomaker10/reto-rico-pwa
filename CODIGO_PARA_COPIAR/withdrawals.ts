...
serve(async (req) => {
  ...
  try {
    ...
    if ((path === '/request' || path.includes('/request')) && req.method === 'POST') {
      ...
      // Calcular saldo disponible (balance - locked)
    ...
    // GET /admin/pending - Obtener retiros pendientes (solo admin)
    ...
    // POST /admin/approve/:id - Aprobar retiro (solo admin)
    ...
    // POST /admin/process/:id - Procesar retiro (enviar USDT)
    if (path.includes('/admin') && path.includes('/process') && req.method === 'POST') {
      ...
      try {
        ...
        // Revertir estado a approved si falla
        await supabase
          .from('withdrawals')
